<?php
require "db.php";

$errors = [];
$success = "";

if($_SERVER["REQUEST_METHOD"] === "POST"){
    $name = trim($_POST["name"]);
    $email = trim($_POST["email"]);
    $course = trim($_POST["course"]);

    if (empty($name)) {
        $errors["name"] = "Name is required.";
    } elseif (strlen($name) < 3) {
        $errors["name"] = "Name must be at least 3 characters.";
    }

    if(empty($email)){
        $errors["email"] = "Email is required.";
    }

    if(empty($course)){
        $errors["course"] = "Couse is required.";
    }

    if(empty($errors)){
        $sql = "INSERT INTO students (name, email, course) VALUES ('$name', '$email', '$course')";

        $result = mysqli_query($conn, $sql);

        if(!$result){
            $errors["sql"] = "Failed to insert in db.";
        }


        $success = "Information submitted successfully";
        $name = $email = $course = "";
    }

}

?>

<html>
    <body>
        <style>
            *{
                margin: 0;
                padding: 0;
                box-sizing: border-box;
            }

            html,body{
                height: 100%;

            }

            body{
                display: flex;
                flex-direction: column;
            }

            .detail{
                flex: 1;
                display: flex;
                justify-content: center;
                align-items: center;
            }

            form {
                background-color: #f2f2f2;
                width: 300px;
                margin: 50px;
                padding: 20px;
                border-radius: 20px;
                box-shadow: 0 10px 25px rgba(0,0,0,0.2);
            }

            label {
                display: inline-block;
                width: 100px;
            }

            input{
                padding: 7px;
                border-radius: 10px;
                width: 100%;

            }

            button{
                
                margin-bottom: 15px;
                padding: 5px;
                border-radius: 10px;
            }

            button:hover{
                background-color: #ddd;
                cursor: pointer;
                transform: scale(1.04);
            }

            .btn{
                display: inline;
                padding: 5px;
                height: 33px;
                border-radius: 10px;
                border: 2px black solid;
            }

            .btn:hover{
                background-color: #ddd;
                cursor: pointer;
                transform: scale(1.04);
            }

            .btn a{
                text-decoration: none;
                color: inherit;
            }

            .btns{
                display: flex;
                justify-content: space-between;
            }
        </style>
        <div class = "detail">
            <form method  = "POST">
                <h3 style = "text-align: center; margin-bottom: 10px;"> Student Information </h3>

                <label>Name: </label><br>
                <input type = "text" name = "name"><br>
                <p style = "color: red;"><?php echo $errors["name"] ?? "" ?></p><br>

                <label> Email: </label><br>
                <input type = "email" name = "email"><br>
                <p style = "color: red;"><?php echo $errors["email"] ?? "" ?></p><br>

                <label> Course: </label><br>
                <input type = "text" name = "course" ><br>
                <p style = "color: red;"><?php echo $errors["course"] ?? "" ?></p><br>

                <div class = "btns">
                <button type = "submit" name = "submit" class = "btns"> Submit </button>
                <?php if (!empty($success)): ?>
                <p style="color: green; text-align: center; margin-top: 10px;"><?= $success ?></p>
                <?php endif; ?>
                </p>

                <p class = "btn" class = "btns"><a href = "db_manipulation.php"> View </a></p>
                </div>
            </form>
        </div>
    </body>
</html>